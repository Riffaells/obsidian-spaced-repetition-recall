...
export default class CommandManager {
    ...
    addCommands() {
        ...
        plugin.addCommand({
            ...
            checkCallback: (checking: boolean) => {
                ...
                if (file && plugin.store.isInTrackedFiles(file.path)) {
                    if (!checking) {
            ...
            checkCallback: (checking: boolean) => {
                ...
                if (file && plugin.store.getTrackedFile(file.path)?.isTrackedNote) {
                    if (!checking) {
                        ...
                        const input = new GetInputModal(plugin.app, t("CMD_INPUT_POSITIVE_NUMBER"));
        ...
        plugin.addCommand({
            ...
            checkCallback: (checking: boolean) => {
                const file = plugin.app.workspace.getActiveFile();
                if (file && plugin.store.getTrackedFile(file.path)?.isTracked) {
                    if (!checking) {
                        const tkfile = plugin.store.getTrackedFile(file.path);
                        const input = new GetInputModal(plugin.app, t("CMD_INPUT_POSITIVE_NUMBER"));
                        input.submitCallback = async (days: number) => {
                            postponeItems(
                                tkfile.cardIDs.map((id) => plugin.store.getItembyID(id)),
                                days,
                            );
                            await plugin.store.save();
                            new Notice(t("CMD_CARDS_POSTPONED", { days: days }));
                            await plugin.sync();
                        };
                        input.open();
                    }
    ...
    addDebugCommands() {
        ...
        plugin.addCommand({
            ...
            callback: () => {
                console.log(plugin.app.workspace.getLeaf().getEphemeralState());
            },
        });
