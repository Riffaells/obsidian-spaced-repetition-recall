...
import { DEFAULT_DECK_NAME, OBSIDIAN_TAG_AT_STARTOFLINE_REGEX } from "src/constants";
...
export class TopicPath {
    ...
    static getFolderPathFromFilename(noteFile: ISRFile, settings: SRSettings): TopicPath {
        ...
        if (result.isEmptyPath && settings.trackedNoteToDecks) {
            ...
            const tagsToReviewList: TopicPath[] = TopicPathList.convertTagListToTopicPathList(
                settings.tagsToReview,
            ).list;

            outer: for (const tagToReview of tagsToReviewList) {
            ...
            if (result.isEmptyPath && settings.dataLocation !== DataLocation.SaveOnNoteFile) {
...
export class TopicPathList {
    ...
    // validTopicPathList is a list of valid tags, such as those from settings.flashcardRules, e.g.
    //      ["#flashcards"]
    //
