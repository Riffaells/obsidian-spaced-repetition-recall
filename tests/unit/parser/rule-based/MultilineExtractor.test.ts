...
Answer here`;
        ...
        it("should return a card for functionally invalid custom stop pattern", () => { // Renamed test
            const text = `Q: What is TypeScript?
Answer here`;

            const config: MultilineConfig = {
                questionLinePattern: "^Q:",
                stopCondition: { type: "custom-pattern", pattern: "\\[invalid\\(regex" }, // Syntactically valid, functionally non-matching
            };

            const cards = extractor.extract(text, config, createContext(), [], []);

            expect(cards).toHaveLength(1); // Expect one card because question + answer are present
            expect(cards[0].front).toBe("Q: What is TypeScript?");
