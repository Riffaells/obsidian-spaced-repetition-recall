    ...
    describe("Reading old DataStore format", () => {
        it("should read old format with no version field", async () => {
            ...
            if (result.isOk) {
                ...
                expect((data.items[0] as any).itemType).toBeDefined();
        ...
        it("should handle old format with missing optional fields", async () => {
            ...
            if (result.isOk) {
                ...
                expect((data.items[0] as any).itemType).toBeDefined();
                expect((data.items[0] as any).deckName).toBeDefined();
        ...
        it("should preserve all data fields during migration", async () => {
            ...
            if (result.isOk) {
                ...
                expect(data.reviewedCounts["2024-01-20"]).toBeDefined();
                expect(data.reviewedCounts["2024-01-20"].new).toBe(1);
                expect(data.reviewedCounts["2024-01-20"].due).toBe(2);
                expect(data.reviewedCardCounts["2024-01-20"]).toBeDefined();
                ...
                for (let i = 0; i < 100; i++) {
                    ...
                    if (result.isOk) {
                        ...
                        data.items.forEach((item) => {
                            expect((item as any).itemType).toBeDefined();
                            expect((item as any).deckName).toBeDefined();
                        });
        ...
        describe("Property 37: Write compatible format", () => {
            it("should write data that can be read back", async () => {
                ...
                for (let i = 0; i < 100; i++) {
                    ...
                    // Write data
                    const writeResult = await storage.write(originalData as any);
 ...
 */
function generateRandomSrsData(seed: number): any {
    const numItems = Math.floor((seed % 10) + 1);
    const numFiles = Math.floor((seed % 5) + 1);
