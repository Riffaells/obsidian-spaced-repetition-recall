        ...
        describe("Property 38: Migration preserves data", () => {
            it("should preserve all user data during migration", async () => {
                ...
                for (let i = 0; i < 100; i++) {
                    ...
                    const originalReviewCounts = oldData.items.map((item: any) => item.timesReviewed);
                    ...
                    if (result.isOk) {
                        ...
                        const migratedFilePaths = migratedData.trackedFiles.map((file) => file.path);
                        expect(migratedFilePaths).toEqual(originalFilePaths);

                        // Verify review history preserved
                        const migratedReviewCounts = migratedData.items.map((item) => item.timesReviewed);
