...
describe("SrsDataMigrator", () => {
    ...
    describe("Property 8: Schema migration application", () => {
        ...
        it("should not migrate data already at current version", async () => {
            ...
            const migrated = await migrator.migrate(currentData as any);

            expect(migrated.version).toBe(2);
            expect((migrated.items[0] as any).itemType).toBe("note");
            expect((migrated.items[0] as any).deckName).toBe("default");
        });
        ...
        it("should migrate from version 0 to current version", async () => {
            ...
            const migrated = await migrator.migrate(v0Data as any);
            ...
            expect((migrated.items[0] as any).itemType).toBe("note");
            expect((migrated.items[1] as any).itemType).toBe("note");

            // v1->v2: Should normalize deckName
            expect((migrated.items[0] as any).deckName).toBe("custom");
            expect((migrated.items[1] as any).deckName).toBe("default");
        ...
        it("should migrate from version 1 to current version", async () => {
            ...
            const migrated = await migrator.migrate(v1Data as any);
            ...
            // v1->v2: Should normalize deckName
            expect((migrated.items[0] as any).deckName).toBe("custom");
            expect((migrated.items[1] as any).deckName).toBe("existing");
        ...
        it("should apply migrations sequentially in order", async () => {
            ...
            expect((migrated.items[0] as any).itemType).toBe("note"); // From v0->v1
            expect((migrated.items[0] as any).deckName).toBe("test"); // From v1->v2
        ...
        it("should handle data with no version field (defaults to 0)", async () => {
            ...
            const migrated = await migrator.migrate(unversionedData as any);
        ...
        it("should handle empty items array", async () => {
            ...
            const migrated = await migrator.migrate(emptyData as any);
        ...
        it("should handle missing items array", async () => {
            ...
            const migrated = await migrator.migrate(noItemsData as any);
        ...
        it("should preserve other fields during migration", async () => {
            ...
            const migrated = await migrator.migrate(dataWithExtraFields as any);
            ...
            expect(migrated.trackedFiles).toEqual([{ path: "test.md" }] as any);
            expect(migrated.queues).toEqual({ some: "queue" } as any);
            expect(migrated.reviewedCounts).toEqual({ "2024-01-01": { new: 1, due: 2 } } as any);
    ...
    describe("Property 9: Migration failure safety", () => {
        ...
        it("should throw MigrationError when data version is newer than current", async () => {
            ...
            await expect(migrator.migrate(futureData as any)).rejects.toThrow(MigrationError);
            await expect(migrator.migrate(futureData as any)).rejects.toThrow(
        ...
        it("should include version information in MigrationError", async () => {
            ...
            try {
                await migrator.migrate(futureData as any);
        ...
        it("should not mutate original data on successful migration", async () => {
            ...
            await migrator.migrate(originalData as any);
        ...
        it("should handle migration errors gracefully", async () => {
            ...
            try {
                await migrator.migrate(problematicData as any);
    ...
    describe("Property 10: Schema version persistence", () => {
        ...
        it("should maintain version field across multiple migrations", async () => {
            ...
            const migratedAgain = await migrator.migrate(migrated as any);
    ...
    describe("Migration specifics", () => {
        describe("v0 -> v1: Add itemType field", () => {
            ...
            it("should preserve existing itemType if present", async () => {
                ...
                expect((migrated.items[0] as any).itemType).toBe("card");
                expect((migrated.items[1] as any).itemType).toBe("note");
            });
        });
        ...
        describe("v1 -> v2: Normalize deckName field", () => {
            ...
            it("should preserve existing deckName over deck field", async () => {
                ...
                expect((migrated.items[0] as any).deckName).toBe("new");
                expect(migrated.items[0]).not.toHaveProperty("deck");
            });
        });
    });
