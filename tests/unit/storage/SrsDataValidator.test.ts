...
describe("SrsDataValidator", () => {
    ...
    describe("validate", () => {
        ...
        it("should detect missing trackedFiles array", () => {
            ...
            expect(result.errors[0].message).toContain("Missing or invalid trackedFiles array");
        });
    ...
    describe("autoFix", () => {
        it("should fix invalid nextReview timestamp", () => {
            ...
            const validation = validator.validate(invalidData as any);
            const fixed = validator.autoFix(invalidData as any, validation.errors);
        ...
        it("should not mutate original data", () => {
            ...
            const originalNextReview = (invalidData.items[0] as any).nextReview;
            const validation = validator.validate(invalidData as any);
            validator.autoFix(invalidData as any, validation.errors);

            // Original data should remain unchanged
            expect((invalidData.items[0] as any).nextReview).toBe(originalNextReview);
        });
